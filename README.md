...
- El link que recibe el usuario apunta a `https://verify.kobot.lat/verify/<token>`.
...
3. Asigna el subdominio `verify.kobot.lat` al proyecto (o el que prefieras).
4. En el **Discord Developer Portal** → tu aplicación → OAuth2 → Redirects,
   agrega: `https://verify.kobot.lat/verify/callback`
   (y puedes quitar la URL vieja de la VPS).
5. En el `.env` del bot (VPS), cambia:
   ```
   WEB_URL=https://verify.kobot.lat
   ```
